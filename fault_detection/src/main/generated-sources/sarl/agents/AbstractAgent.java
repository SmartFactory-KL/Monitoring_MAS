...
public class AbstractAgent extends Agent {
  ...
  protected boolean init(final Object[] parameters, final String assetType) {
    ...
    if ((_size > 1)) {
      ...
      this.comspace = ((OpenEventSpace) _get);
  ...
  @Pure
  protected SubmodelElement getAASElement(final SubmodelElementCollection smc, final String idShort) {
    try {
      final Predicate<SubmodelElement> _function = (SubmodelElement sme) -> {
        return sme.getIdShort().contains(idShort);
      };
      SubmodelElement result = smc.getValue().stream().filter(_function).findFirst().get();
      return result;
    } catch (final Throwable _t) {
      if (_t instanceof NoSuchElementException) {
        Logging _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER = this.$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER();
        String _idShort = smc.getIdShort();
        _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.warning(((("The AAS element " + idShort) + " could not be found in smc ") + _idShort));
      } else {
